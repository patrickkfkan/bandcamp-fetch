  ...
  static parseOptions(html: string): DiscoverOptions {
    ...
    if (!blob) {
      throw new ParseError(
        'Failed to parse discover options: blob not found in data.',
        html
      );
    }
    let parsed;
    try {
      ...
      throw new ParseError(
        'Failed to parse discover options: JSON error.',
        blob,
        error
      );
    }
    ...
    if (options && typeof options === 'object') {
      ...
      if (Array.isArray(options.categories)) {
        result.categories = options.categories.map((cat: any) => ({
          name: cat.label,
          value: cat.id,
          slug: cat.slug
        }));
      ...
      if (Array.isArray(options.genres)) {
        result.genres = options.genres.map((genre: any) => ({
          name: genre.label,
          value: genre.slug,
          id: genre.id
        }));
      ...
      if (Array.isArray(options.slices)) {
        result.sortBys = options.slices.map((s: any) => ({
          name: s.label,
          value: s.slug
        }));
      }
      if (Array.isArray(options.locations)) {
        result.locations = options.locations.map((loc: any) => ({
          name: loc.label,
          value: loc.id
        }));
      }
      if (Array.isArray(options.times)) {
        result.times = options.times.map((t: any) => ({
          name: t.label,
          value: t.id,
          slug: t.slug
        }));
    ...
    throw new ParseError(
      "Failed to parse discover options: blob is missing or has invalid 'appData.initialState' field.",
      parsed
    );
  }
